...
public class DropDownSample : MonoBehaviour
{
    ...
    private void Start()
    {
        text1.text = $"직업: {job_options[0]}";
        text2.text = $"성별: {MaleFemale_options[0]}";
        text3.text = $"진영: {team_options[0]}";
        text4.text = $"종족: {tribe_options[0]}";

        job.ClearOptions(); // 드롭다운의 Option 명단을 제거하는 코드
        MaleFemale.ClearOptions();
        team.ClearOptions();
        tribe.ClearOptions();

        job.AddOptions(job_options); // 준비된 명단에 대한 추가
